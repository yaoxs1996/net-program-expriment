...
int TimerWheel::AddTask(struct TimerTask& task){
    ...
    printf("当前在第%d层、第%d项、时间轮大小为%d\n", layer, slot, m_slot_num);
    if(slot >= m_slot_num){
        slot = slot % m_slot_num;
        layer = (layer+1) % layer;
    }
    //printf("add task in %d %d\n",layer,slot);
    printf("添加任务到第%d层、第%d项\n",layer,slot);
...
int TimerWheel::init()
{
    ...
    printf("定时器设置：%ld ms %ld s\n", new_its.it_interval.tv_nsec / 1000 * 1000, new_its.it_interval.tv_sec);
...
void TimerWheel::tick()
{
    ...
    printf("调度工作时间为：%ld、耗时：%f s\n",newtime, (float)(newtime-oldtime)/CLOCKS_PER_SEC*1000);
    oldtime = newtime;
